...
const Overview: React.FC = () => {
  ...
  // Pagination for sales per day
  const totalSalesPages = Math.ceil(filteredSales.length / productsPerPage);
  const indexOfLastSale = salesPage * productsPerPage;
          ...
          <div className="top-products">
            ...
            <div className="pagination">
              {[...Array(totalProductPages).keys()].map(page => (
                <button
                  key={page + 1}
                  onClick={() => setProductPage(page + 1)}
                  className={productPage === page + 1 ? 'active' : ''}
                >
                  {page + 1}
                </button>
              ))}
          ...
          <div className="sales-per-day">
            ...
            <div className="pagination">
              {[...Array(totalSalesPages).keys()].map(page => (
                <button
                  key={page + 1}
                  onClick={() => setSalesPage(page + 1)}
                  className={salesPage === page + 1 ? 'active' : ''}
                >
                  {page + 1}
                </button>
              ))}
